			...
			if(u.getRole() == Roles.USER) {
				...
				if(org.getName() != null)
				{
					if(!org_name.equals(org.getName()))
					{
						...
						{
							for (VM vm : org.getRsrc().values()) {
								vm.setOrg(org.getName());
							}
							
							for(User us : org.getUsers().values()) {
								us.setOrg(org.getName());
							}
							...
							for (VM vm: Cache.getVms().values()) {
								vm.updateVM(org_name, org.getName());
							}
						}
					}
				}
		...
		get("/api/search" , (req, res) -> {
			...
			if(u.getRole() != Roles.SUPER_ADMIN) {
				if(searched_name.equals(""))
					vms.addAll(Cache.getOrgs().get(u.getOrg()).getRsrc().keySet());
				else
				{
				...
				if(searched_name.equals(""))
					vms.addAll(Cache.getVms().keySet());
				else
				{
...
//		if(u.getRole() != Roles.ADMIN) {
//			res.status(403);
//			msg.addProperty("msg", "User lacks permission to view summary.");
//			return g.toJson(msg);
//		}
//		else
//		{
